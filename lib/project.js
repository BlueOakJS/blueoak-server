...
module.exports = function(loader) {
    ...
    return {
        ...
        initProject: function(callback) {
            ...
            loader.loadConsumers(path.resolve(serverRoot, 'middleware'), 'middleware'); //BO middleware
            loader.loadConsumers(path.resolve(global.__appDir, 'middleware'), 'middleware'); //app middleware

            loader.loadConsumers(path.resolve(serverRoot, 'handlers'), 'handlers'); //BO handlers
            ...
            loader.loadConsumers(path.resolve(serverRoot, 'auth'), 'auth'); //BO authenticators

            var serverProvisions = config.get('server-provisions');
            _.forEach(serverProvisions, function(provision) {
                var root = path.resolve(global.__appDir, 'node_modules');
                loader.loadServices(path.resolve(root, provision, 'services'));
                loader.loadConsumers(path.resolve(root, provision, 'middleware') , 'middleware');
            });
