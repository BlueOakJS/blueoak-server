/*
 * Copyright (c) 2015-2018 PointSource, LLC.
 * MIT Licensed
 */
var tv4 = require('tv4'),
    formats = require('tv4-formats'),
    _ = require('lodash');

tv4.addFormat(formats);


exports.validateParameterType = function (schema, value) {

    //Since data will be coming in from a query parameter, it comes in as a string
    //rather than a JSON object.  So we do our best to cast it to the desired type.
    value = castValueFromString(schema, value);

    var result = tv4.validateMultiple(value, schema);
    return result;
};

/**
 * @typedef {Object} ValidationOptions
 * @property {boolean} [banUnknownProperties=false] - whether to fail validation when undefined properties exist
 * @property {boolean} [failFast=false] - whether to stop validation when the first error is found
 */

/**
 * 
 * @param {Object} schema - the model definition against which to check for validity
 * @param {Object} value - the instance object to check for validity
 * @param {ValidationOptions} [options] - a config object to control behavior of the validation
 */
exports.validateJSONType = function (schema, value, options) {
    var result = tv4.validateMultiple(value, schema, false, _.get(options, 'banUnknownProperties', false));
    return result;
};

exports.getObjectsWithDiscriminator = function (object, modelPath) {
    var map = {};
    var properties = object.properties;
    var duckTypes;
    if (object.type === 'array') {
        if (!object.items) {
            var message = 'OpenAPI array definitions require an "items" property';
            if (modelPath) {
                message += '; at model path: ' + modelPath;
            }
            throw new Error(message);
        }
        properties = object.items.properties;
        if (_.has(object, 'items.allOf')) {
            duckTypes = object.items.allOf;
            _.forEach(duckTypes, function (duckType) {
                _.merge(map, exports.getObjectsWithDiscriminator(duckType));
            });
        }
        if (_.has(object, 'items.discriminator')) {
            map.discriminator = object.items.discriminator;
        }
    }
    else {
        if (_.has(object, 'allOf')) {
            duckTypes = object.allOf;
            _.forEach(duckTypes, function (duckType) {
                _.merge(map, exports.getObjectsWithDiscriminator(duckType));
            });
        }
        if (_.has(object, 'discriminator')) {
            map.discriminator = object.discriminator;
        }
    }
    _.forOwn(properties, function (value, key) {
        if (value.type === 'object' || value.type === 'array' || value.allOf) { //is this a complex type
            map[key] = exports.getObjectsWithDiscriminator(value, 
                modelPath ? [modelPath, key].join('.') : key);
        }
    });
    if (!_.includes(JSON.stringify(map), 'discriminator')) {
        // if map does not include a 'discriminator' property by now, reset to empty object
        map = {};
    }
    return map;
};

/**
 * @param {Object} specs - the swagger spec against which to validate
 * @param {Object} map - the discriminator key map generated by BOS on startup
 * @param {Object} data - the object to be validated
 * @param {ValidationOptions} [options] - a config object to control behavior of the validation
 */
exports.validateIndividualObjects = function (specs, map, data, options) {
    if (!options) {
        options = {};
    }
    var validationErrors = [];
    if (_.isArray(data)) {
        if (map.discriminator) {
            // eslint-disable-next-line lodash/collection-method-value
            _.every(data, function (item) {
                validationErrors = validationErrors.concat(validateObject(specs, item, map.discriminator, options));
                return !(validationErrors.length > 0 && options.failFast);
            });
        }
        
        // eslint-disable-next-line lodash/collection-method-value
        _.every(data, function (item) {
            validationErrors = validationErrors.concat(validateNestedObjects(specs, map, item, options));
            return !(validationErrors.length > 0 && options.failFast);
        });
    }
    else {
        if (map.discriminator) {
            validationErrors = validationErrors.concat(validateObject(specs, data, map.discriminator, options));
        }
        if (!(validationErrors.length > 0 && options.failFast)) {
            validationErrors = validationErrors.concat(validateNestedObjects(specs, map, data, options));
        }
    }
    return validationErrors;
};

/**
 * @param {Object} specs - the swagger spec against which to validate
 * @param {Object} map - the discriminator key map generated by BOS on startup
 * @param {Object} data - the object to be validated
 * @param {ValidationOptions} options - a config object to control behavior of the validation
 */
function validateNestedObjects(specs, map, data, options) {
    var validationErrors = [];
    _.forOwn(map, function (value, key) {
        if (_.isObject(value) && data[key]) { //check for any nested objects
            validationErrors = validationErrors.concat(
                exports.validateIndividualObjects(specs, map[key], data[key], options));
        }
    });
    return validationErrors;
}

/**
 * @param {Object} specs - the swagger spec against which to validate
 * @param {Object} obj - the object to be validated
 * @param {String} disc - the name of the discriminator field in `obj` to use to find the type to evaluate against
 * @param {ValidationOptions} options - a config object to control behavior of the validation
 */
function validateObject(specs, obj, disc, options) {
    var validationErrors = [];
    if (!obj[disc]) { //discriminator not defined for this object
        validationErrors.push({
            message: 'discriminator ' + disc + ' not defined for inheriting model'
        });
    }
    else if (!specs.definitions[obj[disc]]) {
        validationErrors.push({
            message: 'model definition not found for ' + disc +
                ': ALL model definitions must be defined in root swagger document. ' +
                'The built-in $ref compiler can be used to manage your definitions for you.' +
                'Learn more at https://github.com/BlueOakJS/blueoak-server/wiki/Handlers#swagger-ref-compiler'
        });
        //indicate that ALL model definitions must be defined in root swagger document
        //happens automatically during bundling/dereferencing IF all models are referenced at least once in the specs
    }
    else {
        var newErrors = exports.validateJSONType(specs.definitions[obj[disc]], obj, options).errors;
        _.forEach(newErrors, function (newError) {
            newError.model = obj[disc];
        });
        validationErrors = validationErrors.concat(newErrors);
    }
    return validationErrors;
}

function castValueFromString(schema, value) {
    if (schema.type === 'number' || schema.type === 'integer') {
        var orig = value;
        value = Number(value); //cast
        //if value can't be cast to a number, return the original string
        //which will let tv4 create an appropriate error
        if (isNaN(value)) {
            return orig;
        }
    }

    if (schema.type === 'boolean') {
        if (value === 'true') {
            value = true;
        } else if (value === 'false') {
            value = false;
        }
    }

    if (schema.type === 'array') {
        var format = schema.collectionFormat || 'csv';

        //value could already be an array, for example if it was defined 
        //as a default value in the swagger schema
        if (!_.isArray(value)) {
            value = parseArray(value, format);
        }
        for (var i = 0; i < value.length; i++) {
            value[i] = castValueFromString(schema.items, value[i]);
        }
    }

    return value;
}

exports.cast = castValueFromString;

function parseArray(str, format) {

    //str should be an array since multiple query params of the same name were used, e.g. foo=bar1&foo=bar2
    if (format === 'multi') {
        return _.isArray(str) ? str : [str];
    }

    var splitChar = {
        'csv': ',',
        'ssv': ' ',
        'tsv': '\t',
        'pipes': '|'
    }[format];

    return _.split(str, splitChar);
}