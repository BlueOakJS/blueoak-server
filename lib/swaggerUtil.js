...
exports.getObjectsWithDiscriminator = function (object) {
    ...
    if (!JSON.stringify(map).includes('discriminator')) { //if map does not include a 'discriminator' property by now, reset to empty object
        map = {};
    }
    return map;
};
...
function validateObject(specs, obj, disc) {
    ...
    if (!obj[disc]) {//discriminator not defined for this object
        validationErrors.push(
            {
                message: 'discriminator ' + disc + ' not defined for inheriting model ' + JSON.stringify(obj)
            }
            );
    }
        ...
        validationErrors.push(
            {
                message: 'model definition not found for ' + disc + '. ALL model definitions must be defined in root swagger document'
            }
            );
